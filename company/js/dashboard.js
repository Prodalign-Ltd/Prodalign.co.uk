import { auth, db, functions, companyId, TYPE_LABELS } from "./firebase.js";
...
if (!companyId) {
  ...
  if (companyNameEl) companyNameEl.textContent = "Missing company id (?c=...)";
...
const orderReadyDueDate = document.getElementById("orderReadyDueDate");
...
function setActiveItem(item){
  ...
  if (detailPanelEl) {
    if (!item || !activeItemCode) {
      ...
      detailPanelEl.innerHTML = `Click an item in the Tree to view it here.<br/>Use “New Item” to create an item.`;
    } else {
...
function renderCurrentTree() {
  ...
  if (treeTitleEl) treeTitleEl.textContent = (treeMode === "ops") ? "Tree of Operations" : "Tree of Items";
...
function watchItems(){
  ...
  onSnapshot(itemsRef, (snap) => {
    ...
    treeListEl.innerHTML = `<div style="color:#ffb4b4;font-weight:900;">Error loading items: ${err.message}</div>`;
  });
}
...
async function enforceUserCompany(user) {
...
function parseDueDateToTimestamp(dateStr){
  if (!dateStr) return null;
  const [y, m, d] = dateStr.split("-").map(n => parseInt(n, 10));
  if (!y || !m || !d) throw new Error("Invalid due date.");
...
async function sendOrderToOperators({ code, type }){
  ...
  const dueStr = String(orderReadyDueDate.value || "").trim();
  if (!dueStr) throw new Error("Due date is required.");
  const dueDate = parseDueDateToTimestamp(dueStr);
